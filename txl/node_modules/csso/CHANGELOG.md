...
- Fixed wrongly merging of pseudo class and element with the same name, e.g. `:-ms-input-placeholder` and
  `::-ms-input-placeholder` (#383, #416)
...
- Implement AST cloning by adding `clone()` [function](https://github.com/css/csso#cloneast) and
  `clone` [option](https://github.com/css/csso#compressast-options) for `compress()` function (#296)
...
- `minify()` and `minifyBlock()` is always return an object as result now (i.e.
  `{ css: String, map: SourceMapGenerator or null }`)
- `parse()`
...
- `needPosition` parser option is deprecated, `positions` option should be used instead (`needPosition` is used still if
  `positions` option omitted)
...
- compressor refactoring
    ...
    - rename `restructuring` to `restructure` option for `minify()`/`compress()` (`restructuring` is alias for
      `restructure` now, with lower priority)
...
- build browser version with `browserify` (no more `make`, and `web` folder), browser version is available at
